...
router.get('/track/:orderNumber', controller.getOrderByNumber.bind(controller));
router.get('/admin/all', authenticate, authorize('ADMIN'), controller.getAllOrders.bind(controller));
router.get('/admin/:id', authenticate, authorize('ADMIN'), controller.getOrderAdmin.bind(controller));
