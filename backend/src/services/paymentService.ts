...
export function getStripe(): Stripe | null {
  if (!stripeInstance && config.stripe.secretKey) {
    stripeInstance = new Stripe(config.stripe.secretKey, { apiVersion: '2025-02-24.acacia' as Stripe.StripeConfig['apiVersion'] });
  }
...
export class PaymentService {
  ...
  async confirmPayment(paymentIntentId: string, userId: string) {
    ...
    if (paymentIntent.status === 'succeeded') {
      ...
      const receiptUrl = chargeId ? (await stripe.charges.retrieve(chargeId)).receipt_url : null;
  ...
  async handleWebhook(rawBody: string, signature: string) {
    ...
    if (!config.stripe.webhookSecret) throw new BadRequestError('Stripe webhook secret not configured');
