...
function BestsellersContent() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => productService.getProducts({ page, limit: ITEMS_PER_PAGE, isBestSeller: true, sort: 'soldCount', order: 'desc' }),
  });
  ...
  return (
    <div className="container py-8">
      <div className="bg-gradient-to-r from-amber-500/10 via-yellow-500/10 to-amber-500/10 rounded-2xl p-8 mb-8 border border-amber-500/20">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-4 flex-wrap">
          <div className="h-16 w-16 rounded-full bg-amber-500 flex items-center justify-center shrink-0">
            <Award className="h-8 w-8 text-white" />
          </div>
          <div className="flex-1 min-w-0">
            <h1 className="text-3xl md:text-4xl font-bold">Best Sellers</h1>
            <p className="text-muted-foreground mt-1">Our most popular products loved by customers</p>
          </div>
      ...
      {!isLoading && (!data?.products || data.products.length === 0) && (
        <div className="text-center py-16">
          ...
          <p className="text-muted-foreground mb-4">Best selling products will appear here.</p>
