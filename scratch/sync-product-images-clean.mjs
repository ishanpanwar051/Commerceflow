...
// Read user-catalog.ts to extract ALL products and their image URLs
...
// Replace any bad pin hash pattern with realPins
const badPinPattern = /https:\/\/i\.pinimg\.com\/(736x|1200x)\/(?!90\/c0|f8\/6a|7d\/d2|73\/5c|91\/c3|d3\/19|74\/3e|44\/63|9a\/69|61\/95|52\/67|9c\/09|e0\/66|b2\/27|04\/fa|46\/ff|a1\/97|66\/dc|9c\/e1|6c\/8b|60\/bf|9d\/be|7a\/85|c2\/10|b3\/ac|ee\/bf|0c\/97|b8\/36|de\/70|fc\/c9|0e\/2a|1a\/22|97\/28|c5\/31|90\/95|ba\/d5|97\/14|d5\/64|24\/85|f2\/cf|ab\/03|24\/9c|01\/8f|05\/43|20\/d3)[a-zA-Z0-9\/.]+/g;

let idx = 0;
prodText = prodText.replace(badPinPattern, () => {
  const pin = realPins[(idx++) % realPins.length];
  return pin;
});

// Also replace ${photoId} string template bug in audit log if any
prodText = prodText.replace("https://images.unsplash.com/${photoId", "https://images.unsplash.com/photo-1542291026-7eec264c27ff");

fs.writeFileSync(prodImagesPath, prodText, 'utf8');
console.log('✓ Cleaned product-images.ts successfully!');
