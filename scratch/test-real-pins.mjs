...
async function main() {
  const res = await Promise.all(realPins.map(checkUrl));
  const valid = res.filter(r => r.status >= 200 && r.status < 400);
  console.log(`Verified ${valid.length} / ${realPins.length} Pinterest URLs returned 200 OK!`);
}
