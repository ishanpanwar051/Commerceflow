...
adminRouter.patch('/users/:userId/toggle-status', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const prisma = getPrisma();
    const user = await prisma.user.findUnique({ where: { id: req.params.userId }, select: { id: true, isActive: true } });
    if (!user) { res.status(404).json({ success: false, error: 'User not found' }); return; }
    const updated = await prisma.user.update({ where: { id: req.params.userId }, data: { isActive: !user.isActive }, select: { id: true, email: true, firstName: true, lastName: true, role: true, isActive: true } });
...
adminRouter.patch('/users/:userId/role', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const prisma = getPrisma();
    const updated = await prisma.user.update({ where: { id: req.params.userId }, data: { role: req.body.role }, select: { id: true, email: true, firstName: true, lastName: true, role: true } });
...
adminRouter.patch('/inventory/:productId', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const updated = await prisma.inventory.upsert({ where: { productId: req.params.productId }, update: { ...(stock !== undefined && { stock }), ...(lowStockThreshold !== undefined && { lowStockThreshold }) }, create: { productId: req.params.productId, stock: stock || 0, lowStockThreshold: lowStockThreshold || 5 } });
